...
`apply_patch` 是单文件精确文本 patch，不做模糊匹配；`delete_file` 只删除普通文件，`rename_file` 不覆盖既有目标，两者都拒绝目录和符号链接。删除与重命名即使在 `accept_edits` 下也需要显式批准；rename 的来源和目标会同时进入权限 path rule 与 workspace 检查。可选文件检查点启用后，这五种写工具会在执行边界保存 before/after。
...
- 不要把 API Key 直接写进可提交的项目文件。MCP header 可以引用环境变量；插件页读取编辑配置时保留 `${ENV_NAME}` 占位符，不把运行时解析出的 credential 回填成明文。
- `config.effective` / Diagnostics 会把 MCP args、env、headers、URL 脱敏，只显示 secret 来源；它不替代插件页的原始 unresolved 编辑 payload。
