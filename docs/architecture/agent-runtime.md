...
主 Runner 的初始执行额度为 20 次模型迭代，但该数字不是“任务自动失败”的终态。第 15 次会注入收束提示；达到边界或连续 6 次没有真实进展时，Core 使用当前激活模型的隔离、无工具 `turn_continuation_evaluator` 路由判断 `continue / finalize / pause`。评估请求只携带脱敏后的任务意图、Plan/验证、Todo 投影、成功变更/证据摘要、错误类别和预算计数，固定 temperature 0、256 token、8 秒超时且不重试；异常或格式错误统一安全暂停。每次最多追加 4/8/12 次，每个用户 turn 最多评估 3 次，累计硬上限为 56 次。连续 4 次无进展先纠正；重复读取、相同命令/参数/结果、失败、拒绝、超时和纯 Thought 都不算进展，重复申请同一下一步也会被 Core 拒绝。评估原始内容不会进入聊天或 runtime event；私有诊断只在 `stateRoot/control/turn-continuation-diagnostics.jsonl` 保存有界脱敏输出与摘要指纹。

`finalize` 不能覆盖 Plan required verification、Goal completion gate、权限、取消或 workspace containment。只有权威 PlanStep/验证、Todo 投影、工具终态和 Goal gate 一致时，Core 才开放一次无工具最终回复；Provider 即使在该边界返回 tool call 也不会得到执行。`pause` 会把 `Plan.metadata.execution_pause` 持久化：Plan 仍为 `executing`、当前 Step 仍为 `active`，对应 PlanStep Task 与 Todo 投影转为 `pending`；Composer 恢复且重启不会自动写入。只有明确 `/continue`/“继续执行”、Plan 批准或 Permission 恢复才清除暂停并把 Task 恢复为 `running`。Goal 绑定 Plan 时，用户的一次继续操作由 Core 转成受信任的内部控制消息，同时恢复 owner Goal 与 Plan；不会启动一个脱离 Goal 的普通写入 turn。

活动 Plan 的 Todo 由 Core 管理。模型只用稳定 ID `plan:<stepId>` 提交状态，Core 在工具预检阶段注入真实 `plan_id`、`plan_step_id` 与 `approval_generation`，补回遗漏步骤并在 TodoStore 副作用前验证依赖。伪造、过期或越级绑定零副作用拒绝；独立 Todo 保留为临时待办，但不能推进 PlanStep。收尾统计以 PlanStep 为唯一完成事实，Plan 绑定 Todo 不再重复计数。
