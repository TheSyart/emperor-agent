...
每个 actor 的普通 prompt mailbox 默认最多保留 64 条等待命令，容量耗尽时返回结构化 backpressure，不允许无界堆积。忙碌 session 接收新纯文字 prompt 时有两种明确语义：`queue` 等当前 command 收口后启动新 turn；`interject` 绑定当前 running command，并只在 runner 的模型请求前或模型响应后、工具执行前的安全边界消费。Interject 不修改正在飞行的 provider request，也不把半截 token 拼进新请求；连续插话按 receipt 顺序一次消费。附件、Skill 和 MCP inline token 不走 interject，桌面端要求改用普通排队。
