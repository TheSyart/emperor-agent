...
Goal 不会扩大权限。`ask_before_edit`、`accept_edits`、`auto` 三种执行权限与独立 Plan 生命周期的语义仍由现有 permission/control 模块决定；pending Ask/Plan、workspace policy、Plan permission token 和工具 schema 仍在 Goal 之上生效。用户从暂停或等待输入的 Goal 切到独立 Plan 时，renderer 先以 `user_switch_to_plan` 取消 Goal，再开启 Plan；Contract、planning、executing 和 verifying 阶段拒绝快捷切换。旧 Goal ledger 继续保留。
