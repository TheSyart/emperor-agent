...
## External ingest ledger

External Bridge 把可信用户配置保存在 `stateRoot/external_config.json`，把 inbound/outbound 状态保存在 `stateRoot/external/state.json`，把脱敏操作记录写入 `stateRoot/external/audit.jsonl`。`external_config.json` 缺失时固定为 `off`；损坏或未知 schema 在正常启动时隔离并回到网络静默状态。它不接受 project、renderer、模型或 remote payload 覆盖。

state schema v2 使用带全局 revision 和逐记录 revision 的 dedupe ledger，记录 `received`、`accepted`、`dispatched`、`failed`、`dead-letter`，以及 attempt、lease、TTL、turn receipt 和最后错误。旧 schema 的 `seen` pair 会读成有限生命周期的 `dispatched` 记录，避免升级时立即重复已处理事件，同时终止旧集合的无界增长。

同一 Core 进程内，并发到达的相同 `(platform, external_message_id)` 会合并到一个 in-flight dispatch。`accepted` 在调用 mainline 前先持久化 lease；崩溃重启后，lease 有效期内不重放，过期后用稳定 `client_message_id` 按 at-least-once 语义重试。只有 `dispatched` receipt 或 `dead-letter` 是 terminal dedupe；`failed` 可按 attempt policy 恢复。TTL/容量回收优先淘汰 terminal，再按 `updated_at`、revision 的稳定顺序选择；in-flight key、未出队 pending message 和有效 lease 必须跳过。如果全部记录都受保护，允许临时超过容量，不能为了满足上限破坏活跃任务。

当前唯一 production adapter 是 `signed-webhook` operator preview。它只监听配置指定的 loopback 地址，用 key ID、timestamp、nonce 与 HMAC-SHA256 验证严格有界的 v1 payload；secret 只从受信环境变量读取。`eval` 不监听、不发送，`on` 缺 credential 或 owner session 时进入安全降级。Outbound 只向固定 HTTPS URL 发送，不跟随 redirect，使用稳定 `Idempotency-Key` 和 deadline。Ingress、outbound、auth、限流和 dead-letter 审计只保存 digest、reason code、状态码与时延；hot file 达到上限后有界轮转，不保存正文、secret、原始 header 或完整远端地址。

Inbound `dispatched` 在 outbound reply 之前持久化；reply 失败只形成 delivery dead-letter，不能把 inbound 改回可重试或重跑 Agent。这套机制是 at-least-once transport 加本地 dedupe，不等于 exactly-once 外部副作用。仓库仍未内置 Slack、Discord、Teams、邮件或社交平台 adapter，也没有公网 ingress、OAuth 或 renderer 配置入口。
