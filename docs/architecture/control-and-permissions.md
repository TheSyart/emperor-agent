...
2. Runner 暂停当前 turn；renderer 在原聊天时间线的 Ask 卡片中展示选择。
...
Renderer 中 PlanCard 是静态提案与审批卡，不承载执行进度。批准、步骤开始/完成/失败、验证开始/通过/失败及计划终态都按 runtime event 顺序投影为卡片下方的 `plan_activity` 时间线节点；thought、文本与 ToolGroup 继续插在这些里程碑之间。Ask 与 Plan 审批均内嵌原时间线，Composer 在等待交互时禁用，不再维护独立底部审批面板。
