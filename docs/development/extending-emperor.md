...
External inbound 必须先写 `received/accepted` ledger 和 lease，再调用 mainline；成功 receipt 后才能 terminal-dedupe。新增 adapter 时复用 `(platform, external_message_id)` key、稳定 `client_message_id`、TTL/容量回收和 dead-letter policy，不得在 adapter 内另建无界 `seen` 集合。至少覆盖 submit failure 后重启重试、并发重复、lease expiry、dead-letter 和“容量满但 active lease 不可淘汰”。

`signed-webhook` 是 External 的默认关闭 operator preview，不是 provider SDK 模板。扩展 wire adapter 时必须把 transport/auth/rate/schema 留在 adapter，把 dedupe/admission/Mainline/delivery terminal 留在 `ExternalBridgeService`：remote payload 不得选择 session、附件路径、permission、tool 或配置；listener 必须保持 loopback-only，secret 只能来自受信环境，outbound 必须固定 HTTPS、禁止 redirect，并贯穿 Core shutdown `AbortSignal`。Agent reply 只能在 inbound `dispatched` 持久化后发送；outbound 失败必须 dead-letter，禁止重放 Agent effect。同步 strict negative corpus、真实网络 E2E、audit 脱敏/轮转、CoreApi start/stop、effective-config redaction 和 Diagnostics 投影。
