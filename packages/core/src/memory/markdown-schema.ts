export type MemoryMarkdownKind = 'user_profile' | 'global' | 'project' | 'episode'
...
const CANONICAL_SECTIONS: Record<MemoryMarkdownKind, string[]> = {
  ...
  global: ['Long-Term Projects', 'Cross-Project Decisions', 'Open Questions', 'Deprecated'],
  project: ['Project Identity', 'Architecture Notes', 'Build Commands', 'Design Decisions', 'Open Tasks', 'Known Issues', 'Deprecated'],
...
export function appendSectionItem(markdown: string, section: string, item: string): string {
  const existing = sectionBody(markdown, section)
  const nextBody = existing ? `${existing}\n${String(item).trimEnd()}` : String(item).trimEnd()
  return replaceSection(markdown, section, nextBody)
}

export function replaceSection(markdown: string, section: string, content: string): string {
  const lines = splitLines(markdown)
...
function findSectionRange(lines: string[], section: string): { heading: number; bodyStart: number; bodyEnd: number } | null {
  const target = normalizeHeading(section)
  const heading = lines.findIndex((line) => normalizeHeading(sectionHeading(line)) === target)
