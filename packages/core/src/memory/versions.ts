...
export class MemoryVersionStore {
  ...
  constructor(root: string, memoryDir: string, userFile: string, opts?: { maxVersions?: number }) {
  ...
  snapshotPath(path: string, opts?: { target?: MemoryVersionTarget | null; reason?: string }): MemoryVersion | null {
  ...
  list(opts?: { limit?: number; target?: MemoryVersionTarget | null }): MemoryVersion[] {
  ...
  detail(versionId: string): Record<string, unknown> {
    ...
    const diff = unifiedDiff(content.split('\n'), current.split('\n'), `${version.relPath}@${version.id}`, version.relPath)
    return { version: memoryVersionToDict(version), content, currentContent: current, diff }
  }
  ...
  restore(versionId: string): Record<string, unknown> {
    ...
    if (resolvedTarget === null) throw new Error(`memory version path is not allowed: ${version.relPath}`)
    this.snapshotPath(target, { target: resolvedTarget, reason: `pre_restore:${version.id}` })
    MemoryVersionStore.atomicWriteText(target, content.replace(/\s+$/, '') + '\n')
    return { version: memoryVersionToDict(version), path: version.relPath, content: readFileSync(target, 'utf8') }
  }

  payload(opts?: { limit?: number }): Record<string, unknown> {
    const allItems = this.loadIndex()
    return {
      versions: allItems.slice(0, Math.max(1, opts?.limit ?? 30)).map(memoryVersionToDict),
  ...
  private loadIndex(): MemoryVersion[] {
    ...
    try {
      ...
      const corrupt = join(this.versionsDir, `index.corrupt-${Math.trunc(Date.now() / 1000)}.json`)
      try { renameSync(this.indexFile, corrupt) } catch { /* ignore */ }
      return []
    }
    const rows = raw && typeof raw === 'object' && !Array.isArray(raw) ? (raw as Record<string, unknown>).versions : []
  ...
  private writeSnapshot(version: MemoryVersion, content: string): void {
    MemoryVersionStore.atomicWriteJson(join(this.snapshotsDir, `${version.id}.json`), { version: memoryVersionToDict(version), content })
  }

  private readSnapshot(versionId: string): [MemoryVersion, string] {
    const safeId = String(versionId ?? '').trim()
    if (!/^[a-zA-Z0-9_.-]{8,80}$/.test(safeId)) throw new Error('invalid memory version id')
    const path = join(this.snapshotsDir, `${safeId}.json`)
    if (!existsSync(path)) throw new Error(`memory version not found: ${safeId}`)
    const raw = JSON.parse(readFileSync(path, 'utf8'))
    if (!raw || typeof raw !== 'object' || typeof raw.version !== 'object') throw new Error('memory version snapshot is invalid')
  ...
  private resolveRel(relPath: string): string {
    ...
    if (this.targetForPath(target) === null) throw new Error(`memory version path is not allowed: ${relPath}`)
    return target
  }
  ...
  static atomicWriteJson(path: string, payload: Record<string, unknown>): void {
    MemoryVersionStore.atomicWriteText(path, JSON.stringify(payload, null, 2) + '\n')
  }
}
...
function unifiedDiff(a: string[], b: string[], fromFile: string, toFile: string): string {
  ...
  const body: string[] = []
  ops.forEach((op, i) => {
    if (aStart < 0) { aStart = op.aIdx; bStart = op.bIdx }
    if (op.tag === 'equal') { body.push(` ${op.line}`); aCount++; bCount++ }
    else if (op.tag === 'delete') { body.push(`-${op.line}`); aCount++ }
    else { body.push(`+${op.line}`); bCount++ }
    void i
  })
...
interface DiffOp { tag: 'equal' | 'delete' | 'insert'; line: string; aIdx: number; bIdx: number }
...
function diffOps(a: string[], b: string[]): DiffOp[] {
  ...
  let changed = false
  while (i < n && j < m) {
    if (a[i] === b[j]) { ops.push({ tag: 'equal', line: a[i]!, aIdx: i, bIdx: j }); i++; j++ }
    else if (dp[i + 1]![j]! >= dp[i]![j + 1]!) { ops.push({ tag: 'delete', line: a[i]!, aIdx: i, bIdx: j }); i++; changed = true }
    else { ops.push({ tag: 'insert', line: b[j]!, aIdx: i, bIdx: j }); j++; changed = true }
  }
