...
import type { ActiveMemoryBinding, CompactionRunRecord } from './compaction-models'
...
function runRecord(overrides: Partial<CompactionRunRecord> = {}): CompactionRunRecord {
  return {
...
describe('CompactionCursorStore', () => {
  ...
  it('advances only while compacting and never moves compactedUntilSeq backwards', () => {
    ...
    expect(() => store.advance('session_1', { compactedUntilSeq: 10, compactionId: 'compact_1' })).toThrow(/not compacting/)

    store.markCompacting('session_1', { lastHistorySeq: 15, compactionId: 'compact_1' })
    const advanced = store.advance('session_1', { compactedUntilSeq: 10, compactionId: 'compact_1' })
    ...
    store.markCompacting('session_1', { lastHistorySeq: 15, compactionId: 'compact_2' })
    expect(() => store.advance('session_1', { compactedUntilSeq: 9, compactionId: 'compact_2' })).toThrow(/backwards/)
  })

  it('rejects archive progress beyond compactedUntilSeq', () => {
    const store = new CompactionCursorStore(tmp('emperor-compaction-cursor-'))
    store.markCompacting('session_1', { lastHistorySeq: 20, compactionId: 'compact_1' })
    store.advance('session_1', { compactedUntilSeq: 12, compactionId: 'compact_1' })

    expect(() => store.markArchived('session_1', { archivedUntilSeq: 13 })).toThrow(/beyond compactedUntilSeq/)
  ...
  it('exposes a HistoryLog archive gate backed by the session cursor', () => {
    const store = new CompactionCursorStore(tmp('emperor-compaction-cursor-gate-'))
    const gate = store.archiveGate('session_1')

    expect(gate.canArchiveUntil(1)).toBe(false)

    store.markCompacting('session_1', { lastHistorySeq: 5, compactionId: 'compact_1' })
    store.advance('session_1', { compactedUntilSeq: 3, compactionId: 'compact_1' })
...
describe('CompactionLedger', () => {
  it('records started, applied, and failed runs in jsonl plus index', () => {
    ...
    const failed = ledger.recordFailed(runRecord({ compactionId: 'compact_2' }), {
    ...
    const rows = readFileSync(join(root, 'memory', 'compaction', 'runs.jsonl'), 'utf8')
      .trim()
      .split('\n')
      .map((line) => JSON.parse(line) as CompactionRunRecord)
    expect(rows.map((row) => row.status)).toEqual(['started', 'applied', 'failed'])

    const indexPath = join(root, 'memory', 'compaction', 'index.json')
    expect(existsSync(indexPath)).toBe(true)
    const index = JSON.parse(readFileSync(indexPath, 'utf8')) as Record<string, CompactionRunRecord>
    expect(index.compact_1?.status).toBe('applied')
    expect(index.compact_2?.error?.validationErrors).toEqual(['base_hash_mismatch'])
  })
})
