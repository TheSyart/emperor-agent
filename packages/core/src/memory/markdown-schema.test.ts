...
import { appendSectionItem, canonicalSections, replaceSection } from './markdown-schema'

describe('memory markdown schema', () => {
  it('defines canonical sections for each memory kind', () => {
    expect(canonicalSections('user_profile')).toEqual(expect.arrayContaining([
      '基本信息',
      '偏好设置',
      '工作背景',
      'Stable Preferences',
  ...
  it('replaces only the target section body', () => {
    const input = '# Global Long-Term Memory\n\n## Long-Term Projects\n- A\n\n## Open Questions\n- Q\n'
