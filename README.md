...
你可以在设置页保存多个标准接口模型。设置页只管理 Provider、协议、凭证和能力；当前对话使用哪个模型，统一在聊天输入框中选择。切换后，新的请求使用新的模型。旧版双角色字段只用于兼容迁移，不再是当前界面语义。
...
Plan 通过 `/plan on|off|status` 独立管理，不属于权限菜单。上述三种权限都不会关闭路径安全、schema 校验或 Core deny。存在未处理的 Ask 或 Plan 时，执行型 Scheduler、Team 和桌宠 mutation 会被 CoreApi guard 拒绝；Agent Hooks 也不能覆盖 workspace policy 或 Core deny。Goal 同样复用这套规则，不会因为运行时间更长而获得额外权限。
